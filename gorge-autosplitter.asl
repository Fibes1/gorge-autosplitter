...
split
{
	...
	if(settings["Chapter"] && old.chapterCheck!=256 && current.chapterCheck==256) vars.stopwatch.Restart(); // the stopwatch waits 3 seconds after the chapter value is set back to 256 before splitting - this stops the timer at the same time that the igt stops
	...
	if((settings["VHS7Split"]||settings["VHSachSplit"]) && old.vhsCheck==6 && current.vhsCheck==7){
		return true;
	}
